...
public class MechanicalArmBuilder : MonoBehaviour
{
    ...
    public GameObject discoPrefab; // Prefab for the disc to spawn
    ...
    {   
        ...
        // Call the function to check and generate discs
        CheckAndGenerateDisco();
...
// Check if the user clicks on a ball with the VR trigger and spawn a disc
private void CheckAndGenerateDisco()
{
    if (rayInteractor == null || discoPrefab == null) return;

    // Get the ray from the controller
    Ray ray = rayInteractor.Ray;

    // Perform a raycast to detect if the ray hits an object
    if (Physics.Raycast(ray, out RaycastHit hit))
    {
        // Check if the hit object has the tag "Ball"
        if (hit.collider != null && hit.collider.gameObject.CompareTag("JointTag"))
        {
            // Check if the trigger is pressed
            if (OVRInput.GetDown(OVRInput.Button.SecondaryIndexTrigger)) // VR trigger
            {
                // Instantiate the disc prefab at the ball's position
                Instantiate(discoPrefab, hit.collider.transform.position, Quaternion.identity);
                Debug.Log("Disc spawned at the ball's position");
            }
        }
    }
}
